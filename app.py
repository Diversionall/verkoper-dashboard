...
# === Analysepagina: Genummerde Legenda bij Taartdiagram ===
st.title("Top Merken in Batch Beide")
...
# Genummerde legenda op basis van gesorteerde aantallen
nummering = {merk: f"{i+1}. {merk}" for i, merk in enumerate(top_df["Winkel"])}
top_df["Label"] = top_df["Winkel"].map(nummering)

# Plot met nummering alleen in de legenda
fig = px.pie(top_df, values="Aantal", names="Label", title="Top Merken (genummerde legenda)")
st.plotly_chart(fig)
