...
# --- VEILIG TABEL WEERGAVE ---
st.markdown(f"### Resultaten voor winkel: {gekozen_winkel} (pagina {pagina}/{aantal_paginas})")
st.dataframe(pagina_df.round(2), use_container_width=True)

# --- DETAILS ---
with st.expander("🔍 Details per product (optioneel)"):
    if not pagina_df.empty:
        keuzekolommen = [col for col in pagina_df.columns if pagina_df[col].dtype == object]
        if keuzekolommen:
            gekozen_product = st.selectbox("Kies een product voor detailweergave:", pagina_df[keuzekolommen[0]])
            detail = pagina_df[pagina_df[keuzekolommen[0]] == gekozen_product].iloc[0]
            show_product_detail(detail)
        else:
            st.info("Geen geschikte kolom gevonden om detailweergave te tonen.")
    else:
