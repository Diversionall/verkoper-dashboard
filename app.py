...
if uploaded_file:
    ...
    # Rename to clean expected keys
    df.rename(columns={
        "Relevante Marktprijs": "Relevante_Marktprijs",
    ...
    winkel = st.sidebar.multiselect("Winkel", sorted(df["Winkel"].dropna().unique()), default=list(df["Winkel"].dropna().unique()))
    productgroep = st.sidebar.multiselect("Productgroep", sorted(df["Productgroep"].dropna().unique()), default=list(df["Productgroep"].dropna().unique()))
    merknaam = st.sidebar.multiselect("Merknaam", sorted(df["Merknaam"].dropna().unique()), default=list(df["Merknaam"].dropna().unique()))
    ...
    bezoek_labels = ["1.000+", "500-1.000", "250-500", "100-250", "75-100", "50-75", "30-50", "<30"]
    ...
    st.dataframe(df_filtered[["Winkel", "Productgroep", "Merknaam", "price", "Relevante_Marktprijs", "Prijsverschil"]].sort_values("Prijsverschil", ascending=False))
else:
