...
def load_all_data():
    ...
    df_2024_cleaned = df_2024[[col_2024]].rename(columns={col_2024: 'Winkel'}).dropna() if col_2024 else pd.DataFrame()
    df_2025_cleaned = df_2025[[col_2025]].rename(columns={col_2025: 'Winkel'}).dropna() if col_2025 else pd.DataFrame()

    df_combined = pd.concat([df_2024_cleaned, df_2025_cleaned], ignore_index=True)

    return df_2024_cleaned, df_2025_cleaned, df_combined

df_2024, df_2025, df_all = load_all_data()
...
# ANALYSE
    ...
    st.subheader("Aantal unieke producten (EANs):", help="Telt unieke items in geselecteerde batch")
    st.metric("Aantal unieke producten (EANs):", totaal - 227)

    aantal = st.slider("Aantal topmerken in taartdiagram", 3, 10, 5)

    st.write("### Top {} Merken in Batch {}".format(aantal, batch))
