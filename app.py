...
# 2. Analyse-pagina
    ...
    aantal_topwinkels = st.slider("Aantal topwinkels in taartdiagram", 3, 10, 5)
    top_n = df['Winkel'].value_counts().head(aantal_topwinkels).reset_index()
    top_n.columns = ['Winkel', 'Aantal']

    # Nummer de items
    nummering = {winkel: f"{i+1}. {winkel}" for i, winkel in enumerate(top_n['Winkel'])}
    top_n['Label'] = top_n['Winkel'].map(nummering)

    # Maak pie chart met genummerde labels en aangepaste hovertemplate
    fig = px.pie(top_n, names='Label', values='Aantal', title="Top Winkels in Batch Beide")
